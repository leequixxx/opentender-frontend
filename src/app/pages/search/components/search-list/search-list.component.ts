...
export class SearchListComponent {
	...
	public nestedFormat(buckets) {
		...
		if (this.filter.def.nestedType === INestedType.nuts) {
			const result = this.parseByNutsLvl(buckets, 2, 1).map(lvl1 => ({ ...lvl1, children: this.parseByNutsLvl(lvl1.children, 3, 2).map(lvl2 => ({ ...lvl2, children: this.parseByNutsLvl(lvl2.children, 4, 3) })) }));
			if (this.nutsList.length) {
				this.nutsList = [];
			}
			this.updateList(this.nutsList, result);
			return result;
		} else if (this.filter.def.nestedType === INestedType.cpv) {
			const result = this.CPVS;
			if (this.cpvsList.length) {
				this.cpvsList = [];
			}
			this.updateList(this.cpvsList, result);
			return result;
		}

		return buckets;
	}
	private updateList(list, arr): void {
		arr.forEach(i => {
			list.push(i);
			if (i.children) {
				this.updateList(list, i.children);
			}
		});
	};
	private isItemIncludesValue(bucket): boolean {
		return this.getValueTitle(bucket, this.filter).toLowerCase().includes(this.searchValue.toLowerCase());
	}
	...
	get filteredList() {
		...
		return !this.searchValue
			? this.nestedFormat(this.filter.buckets)
			: this.filter.def.nestedType === INestedType.nuts
				? this.nutsList.filter(b => this.isItemIncludesValue(b))
				: this.cpvsList.filter(b => this.isItemIncludesValue(b));
	}
}
